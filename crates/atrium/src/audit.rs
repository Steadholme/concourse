...
//!
//! Atrium only ever emits the logical fields (actor / action / target / severity / detail). No
//! message body, notification text, or feed content rides an event — the audit records WHO loaded
//! their inbox and WHICH federated source was unavailable, never the private payloads themselves.
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::info("inbox.view", "a@b", "/", "ok"));
        }
    ...
    #[test]
    fn event_serializes_to_the_safe_shared_fields() {
        let ev = AuditEvent::warning("inbox.source_unavailable", "ops@w33d.xyz", "chat", "murmur");
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::info("inbox.view", "u", "/", "v1"));
        }
        assert!(
            sink.dropped() > 0,
