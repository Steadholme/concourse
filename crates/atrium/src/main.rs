//! Atrium entry point: init state from env (federated sources + audit), serve.
//!
//! Also exposes a dependency-free `atrium healthcheck` subcommand used as the container
...
async fn main() {
    ...
    let state = match atrium::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
    ...
    let app = atrium::app(state);

    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .unwrap_or_else(|e| panic!("failed to bind {addr}: {e}"));

    tracing::info!(%addr, "Atrium listening (unified activity inbox)");
    axum::serve(listener, app).await.expect("server error");
}

/// GET `/healthz` over a raw TCP socket. Returns process exit code (0 = healthy).
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:9070".to_string());
    // Healthcheck always talks to the loopback regardless of the bind interface.
    let port = bind_addr.rsplit(':').next().unwrap_or("9070");
