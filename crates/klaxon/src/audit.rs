...
impl AuditEvent {
    ...
    /// `info`-severity event (normal lifecycle: a notification fan-out).
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::info("notify.fanout", "u", "src", "ok"));
        }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::info("notify.fanout", "u", "src", "v1"));
        }
        assert!(
            sink.dropped() > 0,
