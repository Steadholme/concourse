...
//! annunciator + dispatches fragments into their `summary` + `columns` slots WITHOUT a full
...
pub struct InboxPayload {
    ...
    pub summary: String,
    /// Rendered dispatches-table HTML (server-escaped) for `#columns-slot`.
    pub columns: String,
}

/// `GET /api/inbox` — the aggregated inbox as JSON for the client poll. Scoped to the injected
/// viewer subject; a request with no gateway identity gets the same neutral signed-out fragments the
/// page renders (never a fake, signed-in-looking empty inbox).
pub async fn api_inbox(
    ...
    // No gateway identity -> nothing to federate; return the signed-out fragments (matches the page).
    let Some(sub) = auth::subject(&headers) else {
        let empty = empty_inbox();
        let view = inbox::view(&empty, &Default::default(), &filter);
        return json_response(payload(&view, now, &token, &filter, false));
    };
    ...
    json_response(payload(&view, now, &token, &filter, true))
}

/// Build the JSON payload by reusing the page's own render helpers. `signed_in = false` yields the
/// neutral signed-out fragments (no counts, no forms) instead of a fake, signed-in-looking inbox.
fn payload(
    inbox: &Inbox,
    now: i64,
    token: &str,
    ...
    InboxPayload {
        ...
        summary: render_summary(inbox, filter, signed_in),
        columns: render_columns(inbox, now, token, filter, signed_in),
    }
}

/// Wrap the payload as JSON that (like the private page) is never cached beyond this response. The
/// frozen `{summary, columns, total_unread}` key set is unchanged.
fn json_response(payload: InboxPayload) -> Response {
    (
...
mod tests {
    ...
    async fn api_inbox_returns_json_fragments_and_total() {
        ...
        assert_eq!(
            res.headers()
                ...
                .unwrap_or_default(),
            "private, no-store",
            "the viewer-private poll carries the frozen cache suppression"
        );
        ...
        assert!(
            summary.contains(r#"class="annun""#),
            "annunciator fragment present"
        );
        assert!(
            columns.contains("#general"),
            "chat slip rendered in dispatches fragment"
        );
        assert!(
            columns.contains("Notifications — unavailable in this view"),
            "down office is named as unavailable in this view"
        );
        assert!(
            columns.contains(r#"class="dispatches""#),
            "single dispatches fragment returned"
        );
    }

    #[tokio::test]
    async fn api_inbox_without_identity_renders_signed_out_shell() {
        ...
        assert!(
            json["summary"]
                .as_str()
                .unwrap()
                .contains("No viewer on this request"),
            "signed-out annunciator returned for an unidentified poll"
        );
        assert!(
            json["columns"]
                .as_str()
                .unwrap()
                .contains("There is nothing personal to show on this page."),
            "signed-out dispatches copy returned, not a fake empty inbox"
        );
    }
    ...
    #[tokio::test]
    async fn api_poll_dispatches_carry_native_forms_and_existing_cookie_token() {
        let res = crate::app(state())
            .oneshot(
                Request::builder()
        ...
        let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
        let columns = json["columns"].as_str().unwrap();
        assert!(columns.contains(r#"class="slip__form""#));
