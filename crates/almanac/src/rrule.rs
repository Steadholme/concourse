...
/// Return whether `occurrence_start` is one of the real starts generated by `rrule` from
/// `dtstart`. This is the single recurrence-membership definition used by mutation validation and
/// Store-side series reconciliation.
pub fn occurrence_belongs_to_series(dtstart: i64, rrule: &str, occurrence_start: i64) -> bool {
    if occurrence_start < dtstart {
        return false;
    }
    RRule::parse(rrule).is_some_and(|rule| {
        rule.expand(dtstart, 0, occurrence_start, occurrence_start)
            .into_iter()
            .any(|(start, _)| start == occurrence_start)
    })
}
...
mod tests {
    ...
    #[test]
    fn occurrence_membership_uses_the_same_expansion_rules() {
        let start = ms("2026-06-01T09:00");
        let monday = start;
        let wednesday = ms("2026-06-03T09:00");
        let next_monday = ms("2026-06-08T09:00");
        let after_count = ms("2026-06-10T09:00");

        assert!(occurrence_belongs_to_series(
            start,
        ...
        assert!(!occurrence_belongs_to_series(start, "", monday));
    }
}
